...
build_authorizer(uint32_t service_id) const 
{
  ...
  if (krb_auth) {
    krb_auth->generate_authorizer(cct->_conf->name, global_id);
  }
  return krb_auth;
}
...
auto KrbClientHandler::build_request(bufferlist& buff_list) const
{
  constexpr auto SUBSYSTEM_ID(20);
...
auto KrbClientHandler::handle_response(int ret, bufferlist::iterator& buff_list) 
{
  constexpr auto SUBSYSTEM_ID(20);
  ...
  gss_mechs_wanted.elements = &GSS_API_KRB5_OID_PTR;
  ...
  if (m_gss_credentials == GSS_C_NO_CREDENTIAL) {
    ...
    *_dout << dendl;
    ...
    gss_buffer_in.length = krb_token.m_token_blob.lenght();
