...
int32_t LDAPMechanismBase::ldap_connect(const std::string& ldap_uri,
  ...
  /*  It allocates an LDAP structure but does not open an initial connection.
    int ldap_initialize(ldp, uri)
    LDAP **ldp;
    char *uri;
  */
  constexpr auto ldap_protocol_version(3);
  m_ldap_status = ldap_initialize(&m_ldap_connection_handler, ldap_uri.c_str());
  if (m_ldap_connection_handler) {
...
std::string LDAPMechanismBase::ldap_build_uri(const std::string& ldap_uri,
                                              const ldap_utils::LDAPSSLOption&
                                                    ldap_ssl_option) {
  if (ldap_uri.empty() ||
      (ldap_uri.find(ldap_utils::LDAP_SERVICE_NAME) != std::string::npos) ||
      (ldap_uri.find(ldap_utils::LDAPS_SERVICE_NAME) != std::string::npos)) {
    return ldap_uri;
  }

  auto uri_prefix(ldap_utils::LDAP_SERVICE_NAME);
  if (ldap_ssl_option == ldap_utils::LDAPSSLOption::ON) {
    uri_prefix = ldap_utils::LDAPS_SERVICE_NAME;
  }
  return (uri_prefix + ldap_utils::PROTO_SEP + ldap_uri);
}

LDAPMechanismBase::LDAPMechanismBase() : m_ldap_connection_handler(nullptr),
                                         m_ldap_status(0) { }

LDAPMechanismBase::LDAPMechanismBase(const auth_mechanisms::AuthOptions& authOptions) :
    LDAPMechanismBase::LDAPMechanismBase() {
  m_authentication_options = authOptions;
}

LDAPMechanismBase::LDAPMechanismBase(const std::string& ldap_uri,
                                     const ldap_utils::LDAPSSLOption& ldap_ssl_option) :
    LDAPMechanismBase() {
  m_ldap_uri    = ldap_uri; 
...
LDAPEntries_t LDAPMechanismBase::ldap_search(const std::string& ldap_base_search,
  ...
  if ((m_ldap_status == LDAP_SUCCESS) &&
      ldap_count_entries(m_ldap_connection_handler, ldap_result) != common_utils::ZERO) {
    ...
    for (LDAPMsgPtr_t ldap_entry(ldap_first_entry(m_ldap_connection_handler,
      ...
      for (charPtr_t ldap_attribute(ldap_first_attribute(m_ldap_connection_handler,
                                                         ldap_entry,
                                                         &ldap_basic_encode_rules));
           ldap_attribute;
           ldap_attribute = ldap_next_attribute(m_ldap_connection_handler,
                                                ldap_entry,
                                                ldap_basic_encode_rules)) {
        if (LDAPBerValPtr_t* ldap_values =
          ...
          if (value_count) {
            auto itr = last_ldap_dn.ldap_entry_push_back(
                new LDAPModify(ldap_attribute, ldap_utils::LDAPModifyOperation::ADD));
...
int32_t LDAPMechanismBase::ldap_modify_entry_dn(const std::string& ldap_old_entry_dn,
...
int32_t LDAPMechanismBase::ldap_compare_attribute(const std::string& ldap_attribute,
                                                  const std::string& ldap_attribute_value)
                                                  const {
  /*  Performs an LDAP compare operation synchronously.
    ...
    LDAPControl **clientctrls );
  */
  return common_utils::auth_trap_error(LDAP_COMPARE_TRUE == (ldap_compare_ext_s(m_ldap_connection_handler,
...
LDAPModify::LDAPModify(const std::string& ldap_modification_type,
    ...
    if (mod_values) {
      ...
      if ((itr_end > itr_begin) && (itr_end != itr_begin)) {
        *itr_position = ldap_value;   //duplicate_ptr_char()
        ...
        std::memset(new_values, common_utils::ZERO, sizeof(charPtr_t) * new_size);
      ...
      std::memset(mod_values, common_utils::ZERO, sizeof(charPtr_t) * m_ldap_modify_size);
    ...
    if (mod_bvalues) {
      ...
      if ((itr_end > itr_begin) && (itr_end != itr_begin)) {
        *itr_position = (new LDAPBasicEncodeRuleValue(ldap_value));
        ...
        std::memset(new_bvalues, common_utils::ZERO, sizeof(LDAPBerValPtr_t) * new_size);
        std::copy(itr_begin, itr_end, new_bvalues);
        delete[] mod_bvalues;
        mod_bvalues = new_bvalues;
        mod_bvalues[m_ldap_modify_size - 1] = (new LDAPBasicEncodeRuleValue(ldap_value));
        m_ldap_modify_size = new_size;
      }
    }
    else {
      ...
      std::memset(mod_bvalues, common_utils::ZERO, sizeof(LDAPBerValPtr_t) * m_ldap_modify_size);
      mod_bvalues[common_utils::ZERO] = (new LDAPBasicEncodeRuleValue(ldap_value));
...
void LDAPModify::ldap_modify_clear() {
  if (mod_bvalues) {
    if (mod_op & static_cast<int>(ldap_utils::LDAPModifyOperation::BIN_VALUES)) {
      for (LDAPBerValPtr_t* bvalue = mod_bvalues; bvalue && *bvalue; ++bvalue) {
...
bool LDAPModify::is_it_operation(
  ...
  return ((mod_op & static_cast<int>(ldap_utils::LDAPModifyOperation::OPERATION)) ==
      static_cast<int>(ldap_modification_operation));
}

const charPtr_t* LDAPModify::get_string_value() const {
  return (mod_op & static_cast<int>(ldap_utils::LDAPModifyOperation::BIN_VALUES)) ?
         nullptr : mod_values;
}
...
void LDAPEntry::ldap_entry_append(
  ...
  static_assert(std::is_same_v<std::string, T> ||
                ...
                std::is_same_v<std::vector<std::vector<char>>, T>, "Not supported type!");
...
T LDAPEntry::get_string_value(
  ...
  static_assert(std::is_same_v<std::string, T> ||
                std::is_same_v<std::vector<std::string>, T>, "Not supported type!");
  ...
  if (ldap_modification.mod_bvalues) {
    if (ldap_modification.mod_op &
      ...
      for (LDAPBerValPtr_t *bvalue = ldap_modification.mod_bvalues;
        ...
        if (is_it_binary) {
          std::vector<char> base64_tmp(ldap_utils::base64_encode((*bvalue)->bv_val,
                                                                 (*bvalue)->bv_len));
          std::copy(std::begin(base64_tmp),
                    std::end(base64_tmp),
                    std::ostream_iterator<char>(out_stream,
                                                common_utils::EMPTY_STR.c_str()));
        }
        else {
