...
std::string str_trim(const std::string& str_to_trim) {
  const auto wsfront=std::find_if_not(begin(str_to_trim), end(str_to_trim),
                                      [](int c) { return std::isspace(c); });
  const auto wsback=std::find_if_not(rbegin(str_to_trim), rend(str_to_trim),
...
auto is_valid_hostname(const std::string& host_name_rfc1123) {
  const std::string host_fqdn_filter("^([a-z0-9]|[a-z0-9][a-z0-9\\-]{0,61}[a-z0-9])\\(\\.([a-z0-9]|[a-z0-9][a-z0-9\\-]{0,61}[a-z0-9]))*$");
...
auto is_valid_ipaddress(const std::string& host_ipaddress) {
  const std::string ipaddr_filter("^(([01]?[0-9]?[0-9]|2([0-4][0-9]|5[0-5]))\\.){3}([01]?[0-9]?[0-9]|2([0-4][0-9]|5[0-5]))$");
